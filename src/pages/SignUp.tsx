import { Text, View, Button, StyleSheet, Dimensions, TextInput, TouchableOpacity, DeviceEventEmitter } from "react-native";
...
function SignUpLogin({ navigation }: SignInScreenProps2) {
  ...
  return (
      <KeyboardAwareScrollView
        ...
        <View>
          <Text style={(email ? [styles.nextButton, styles.nextButton2] : styles.nextButton)} onPress={() => navigation.navigate('SignUpPassWord')} >
            다음
          </Text>
        </View>
      </KeyboardAwareScrollView >
...
function SignUpHome({ navigation }: SignInScreenProps) {
  ...
  return (
    <View style={styles.container} >
      ...
      <TouchableOpacity activeOpacity={1} style={styles.termsBox} onPress={onClickCheckAll}  >
        <Icon style={styles.checkBox} name="checksquareo" size={22} color={checkAll ? "#000" : "#848484"} />
        <Text style={styles.agreeText} >모두 동의</Text>
      </TouchableOpacity>
      <Text style={styles.bottomBar} ></Text>
      <TouchableOpacity activeOpacity={1} style={styles.termsBox} onPress={onClickPrivacy}  >
        <Icon style={styles.check} name="check" size={15} color={checkPrivacy ? "#000" : "#848484"} />
        <Text style={{ color: "#000" }} >[필수] 개인정보 </Text>
      </TouchableOpacity>
      <TouchableOpacity activeOpacity={1} style={styles.termsBox} onPress={onClickTerms}  >
        <Icon style={styles.check} name="check" size={15} color={checkTerms ? "#000" : "#848484"} />
        <Text style={{ color: "#000" }} >이용약관 </Text>
      </TouchableOpacity>
      <Text style={(checkPrivacy && checkTerms) ? [styles.nextButton, styles.nextButton2] : styles.nextButton} onPress={() => navigation.navigate('SignUpLogin')} >
        다음
      </Text>
    </View>
  )
