...
function CalculatePage() {
  ...
  const [totalData, setTotalData] = useState<any>({
    ...
    userBuy: [
      {amount: 214, type: 'black'},
      {amount: 120, type: 'red'},
      {amount: 113, type: 'gold'},
    ],
  });
  ...
  useEffect(() => {
    const getTotalAccount = async () => {
      try {
        const TotalAccount = await axios.get(
          `${Config.API_URL}/account/issued`,
        );
        ...
        let result = {
          prize: (prize / total) * 100,
          userBuy: (userBuy / total) * 100,
        };
        setChartData(result);
      } catch (error) {
        console.error(error);
    ...
    const userAccount = async () => {
      try {
        const monthAccount = await axios.get(
          `${Config.API_URL}/account/consumption`,
        );
        ...
        for (let j = 0; j < result.length; j++) {
          result[j]['data'] = Math.floor(
            (result[j]['amount'] / total) * 100 * 1.25,
          );
        }
    ...
    const monthAccount = async () => {
      ...
      try {
        const monthAccount = await axios.get(
          `${Config.API_URL}/account/issued/details/monthly?date=${day}'`,
        );
        let monthdata = {};

        let total =
          monthAccount.data[0]['amount'] +
          monthAccount.data[1]['amount'] +
          monthAccount.data[2]['amount'];
        ...
        monthdata = {
          black: blackarr[0]['amount'],
          red: redarr[0]['amount'],
          gold: goldarr[0]['amount'],
        };
  ...
  const data3: PieData = {
    dataSets: [
      {
        values: [
          {value: chartData3['black']},
          {value: chartData3['red']},
          {value: chartData3['gold']},
        ],
        label: '',
        config: {
          colors: [
          ...
          <View style={styles.containerBox}>
            ...
            <View style={{flex: 2.3, flexDirection: 'row'}}>
              <View style={{flex: 1, paddingLeft: 25 * heightScale}}>
                <View style={{flexDirection: 'row',alignItems:'center',marginBottom:heightScale* 5}}>
                  <View style={{marginRight:heightScale*15}}>
                    <Rectangle type="big" scale={10} color={'#8CB2EA'} />
                  </View>
                  <Text style={styles.fontStyle4}>유저 충전</Text>
                </View>
                <View>
                  <Text style={[styles.fontStyle3, styles.marginLeft]}>
                    Black Ticket: {totalData['prize'][0]['amount']}장
                  </Text>
                </View>
                <View style={{flexDirection: 'row'}}>
                  <Text style={[styles.fontStyle3, styles.marginLeft]}>
                    Red Ticket: {totalData['prize'][1]['amount']}장
                  </Text>
                </View>
              </View>
              <View style={{flex: 1, paddingLeft: 20 * heightScale}}>
                ...
                <View style={{flexDirection: 'row'}}>
                  <Text style={[styles.fontStyle3, styles.marginLeft]}>
                    Black Ticket: {totalData['userBuy'][0]['amount']}장
                  </Text>
                </View>
                <View style={{flexDirection: 'row'}}>
                  <Text style={[styles.fontStyle3, styles.marginLeft]}>
                    Red Ticket: {totalData['userBuy'][1]['amount']}장
                  </Text>
                </View>
              </View>
            </View>
              ...
              <View style={styles.rodBox}>
                {chartData2.map((val, key) => (
                  <TouchableOpacity
                    ...
                    {key == selectIndex && (
                      <Text style={{color: '#FFF48D'}}>{val.amount}</Text>
                    )}
                    <View
                      style={[
                        styles.rod,
                        {height: val.data * heightScale},
                      ]}></View>
                    <Text
                      style={[
                        styles.fontStyle3,
          ...
          <View style={styles.containerBox3}>
            ...
            <View style={{flex: 7}}>
              <View
                ...
                <Rectangle type="big" scale={10} color={'#fff'} />
                <Text
                  style={[
                    styles.fontStyle5,
                    {marginHorizontal: 10 * heightScale},
                  ]}>
                  합계: ₩{' '}
                  {(
                    monthData['black'] * 50000 +
                    monthData['red'] * 10000 +
                    monthData['gold'] * 300000
                  ).toLocaleString()}
                </Text>
                <Rectangle type="big" scale={10} color={'#fff'} />
              </View>
              <View style={{marginTop: 40 * heightScale}}>
                <View
                  ...
                  <View style={styles.rectangleStyle}>
                    <Rectangle type="big" scale={10} color={'#000'} />
                  </View>
                  <Text style={styles.fontStyle4}>
                    Black: {chartData3['black'].toFixed(2)}%
                  </Text>
                </View>
                <View style={styles.marginLeft}>
                  <Text style={[styles.fontStyle3, styles.marginLeft]}>
                    {monthData['black']}장 → ₩{' '}
                    {(monthData['black'] * 50000).toLocaleString()}
                  </Text>
                </View>
              </View>
              ...
              <View style={{marginTop: 25 * heightScale}}>
                <View
                  ...
                  <View style={styles.rectangleStyle}>
                    <Rectangle type="big" scale={10} color={'#C53F3F'} />
                  </View>
                  <Text style={styles.fontStyle4}>
                    Red: {chartData3['red'].toFixed(2)}%
                  </Text>
                </View>
                <View style={styles.marginLeft}>
                  <Text style={[styles.fontStyle3, styles.marginLeft]}>
                    {monthData['red']}장 → ₩{' '}
                    {(monthData['red'] * 10000).toLocaleString()}
                  </Text>
                </View>
              </View>
              ...
              <View style={{marginTop: 25 * heightScale}}>
                <View
                  ...
                  <Text style={styles.fontStyle4}>
                    Gold: {chartData3['gold'].toFixed(2)}%
                  </Text>
                </View>
                <View style={styles.marginLeft}>
                  <Text style={[styles.fontStyle3, styles.marginLeft]}>
                    {monthData['gold']}장 → ₩{' '}
                    {(monthData['gold'] * 300000).toLocaleString()}
                  </Text>
                </View>
              </View>
            </View>
