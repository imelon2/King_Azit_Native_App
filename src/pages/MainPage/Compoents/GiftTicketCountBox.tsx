  ...
  return (
    <View style={styles.countBox}>
      <Pressable
        ...
        disabled={canAdd}>
        <IconAntDesign size={heightScale * 16} name="minus" color={canAdd ? '#787878' : '#FFFFFF'} />
      </Pressable>
      {/* count */}
      <View style={{flex: 1.5, alignItems: 'center', justifyContent: 'center'}}>
        <Text style={{fontSize: heightScale * 18, color: '#ffffff'}}>{props.ticket}</Text>
      </View>
      {/* plus */}
      <Pressable
        ...
        onPress={() => props.setTicket(props.ticket + 1)}
        disabled={canMius}>
        <IconAntDesign size={heightScale * 16} name="plus" color={canMius ? '#787878' : '#FFFFFF'} />
      </Pressable>
    </View>
  );
};
